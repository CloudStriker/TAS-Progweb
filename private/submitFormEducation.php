<?php 
    ...
    if($_POST){
        ...
        if($validate){
            ...
            $sql = "INSERT INTO education (label, start, end, title, info, user) VALUES ('$label', '$start', '$end', '$title', '$info', '".$_SESSION['users']['user']."' )";
            echo $sql;
            if(mysqli_query($con, $sql)){
                header("location:listEducation.php?msg=1");
            }else{
                header("location:formEducation.php?msg=0");
            }
         }else{
            $_SESSION['msg'] = $msg;
            header("location:formEducation.php?msg=0");
